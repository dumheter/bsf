...
namespace bs {
void BuiltinResourcesHelper::importAssets(
    const nlohmann::json &entries, const Vector<bool> &importFlags,
    const Path &inputFolder, const Path &outputFolder,
    const SPtr<ResourceManifest> &manifest, AssetType mode,
  ...
  struct QueuedImportOp {
    QueuedImportOp(const TAsyncOp<HResource> &op, const Path &outputPath,
  ...
  auto importResource = [&](const nlohmann::json &entry) {
    ...
    if (mode == AssetType::Normal)
      uuidStr = entry.value("UUID", "");
    else if (mode == AssetType::Sprite)
      uuidStr = entry.value("TextureUUID", "");
    ...
    SPtr<ImportOptions> importOptions =
        gImporter().createImportOptions(filePath);
    if (importOptions != nullptr) {
      if (rtti_is_of_type<TextureImportOptions>(importOptions)) {
        ...
        ShaderDefines defines =
            RendererMaterialManager::_getDefines(relativePath);
    ...
    TAsyncOp<HResource> op =
        gImporter().importAsync(filePath, importOptions, UUID);
    queuedOps.emplace_back(op, outputPath, entry);
  };

  auto generateSprite = [&](const HTexture &texture, const String &fileName,
                            const UUID &UUID) {
    Path relativePath = fileName;
    ...
    HResource spriteTex =
        gResources()._createResourceHandle(spriteTexPtr, UUID);

    Resources::instance().save(spriteTex, outputPath, true, compress);
    manifest->registerResource(spriteTex.getUUID(), outputPath);
  };

  auto generateAnimatedSprite = [&](const HTexture &texture,
                                    const String &fileName, const UUID &UUID,
                                    SpriteAnimationPlayback playback,
                                    const SpriteSheetGridAnimation &animation) {
    Path relativePath = fileName;
    Path outputPath = spriteOutputFolder + relativePath;
  ...
  while (!queuedOps.empty()) {
    for (auto iter = queuedOps.begin(); iter != queuedOps.end();) {
      ...
      if (outputRes != nullptr) {
        Resources::instance().save(outputRes, importOp.outputPath, true,
                                   compress);
        ...
        if (rtti_is_of_type<Shader>(outputRes.get())) {
          ...
          if (dependencies != nullptr) {
            SPtr<ShaderMetaData> shaderMetaData =
                std::static_pointer_cast<ShaderMetaData>(shader->getMetaData());

            nlohmann::json dependencyEntries;
            if (shaderMetaData != nullptr &&
                shaderMetaData->includes.size() > 0) {
              for (auto &include : shaderMetaData->includes) {
                Path includePath = include.c_str();
                if (include.substr(0, 8) == "$ENGINE$" ||
                    include.substr(0, 8) == "$EDITOR$") {
                  if (include.size() > 8)
        ...
        if (isIcon) {
          ...
          if (mode == AssetType::Normal) {
            iconData.TextureUUIDs[0] = entry.value("UUID48", "");
            iconData.TextureUUIDs[1] = entry.value("UUID32", "");
            iconData.TextureUUIDs[2] = entry.value("UUID16", "");
          } else if (mode == AssetType::Sprite) {
            iconData.TextureUUIDs[0] = entry.value("TextureUUID48", "");
            iconData.TextureUUIDs[1] = entry.value("TextureUUID32", "");
            iconData.TextureUUIDs[2] = entry.value("TextureUUID16", "");

            iconData.SpriteUUIDs[0] = entry.value("SpriteUUID48", "");
            iconData.SpriteUUIDs[1] = entry.value("SpriteUUID32", "");
            iconData.SpriteUUIDs[2] = entry.value("SpriteUUID16", "");
          }
  ...
  auto saveTexture = [&](auto &pixelData, auto &path, std::string &uuid) {
    ...
    HResource texture =
        gResources()._createResourceHandle(texturePtr, UUID(uuid.c_str()));
  ...
  for (UINT32 i = 0; i < (UINT32)iconsToGenerate.size(); i++) {
    ...
    HTexture tex48 =
        saveTexture(scaled48, outputPath48, iconsToGenerate[i].TextureUUIDs[0]);
    HTexture tex32 =
        saveTexture(scaled32, outputPath32, iconsToGenerate[i].TextureUUIDs[1]);
    HTexture tex16 =
        saveTexture(scaled16, outputPath16, iconsToGenerate[i].TextureUUIDs[2]);

    if (mode == AssetType::Sprite) {
      generateSprite(tex48, iconsToGenerate[i].name + "48",
                     UUID(iconsToGenerate[i].SpriteUUIDs[0].c_str()));
      generateSprite(tex32, iconsToGenerate[i].name + "32",
                     UUID(iconsToGenerate[i].SpriteUUIDs[1].c_str()));
      generateSprite(tex16, iconsToGenerate[i].name + "16",
                     UUID(iconsToGenerate[i].SpriteUUIDs[2].c_str()));
    }
  }
}

void BuiltinResourcesHelper::importFont(
    const Path &inputFile, const String &outputName, const Path &outputFolder,
    const Vector<UINT32> &fontSizes, bool antialiasing, const UUID &UUID,
    const SPtr<ResourceManifest> &manifest) {
  SPtr<ImportOptions> fontImportOptions =
      Importer::instance().createImportOptions(inputFile);
  if (rtti_is_of_type<FontImportOptions>(fontImportOptions)) {
    FontImportOptions *importOptions =
        static_cast<FontImportOptions *>(fontImportOptions.get());

    importOptions->fontSizes = {fontSizes};
    importOptions->renderMode = antialiasing ? FontRenderMode::HintedSmooth
                                             : FontRenderMode::HintedRaster;
  } else
    return;

  HFont font =
      Importer::instance().import<Font>(inputFile, fontImportOptions, UUID);
  ...
  // Save font texture pages as well. TODO - Later maybe figure out a more
  ...
  for (auto &size : fontSizes) {
    ...
    for (auto tex : fontData->texturePages) {
      texPageOutputPath.setFilename(fontName + u8"_" + toString(size) +
                                    u8"_texpage_" + toString(pageIdx) +
                                    u8".asset");
...
Vector<bool> BuiltinResourcesHelper::generateImportFlags(
    const nlohmann::json &entries, const Path &inputFolder,
    time_t lastUpdateTime, bool forceImport, const nlohmann::json *dependencies,
    const Path &dependencyFolder) {
      ...
      else if (dependencies != nullptr) // Check dependencies
      {
        ...
        if (iterFind != dependencies->end()) {
          for (auto &dependency : *iterFind) {
            ...
            Path dependencyPath =
                dependencyFolder + Path(dependencyName.c_str());

            time_t lastModifiedDep =
                FileSystem::getLastModifiedTime(dependencyPath);
...
bool BuiltinResourcesHelper::updateJSON(const Path &folder, AssetType type,
  ...
  auto checkForChanges = [&](const Path &filePath) {
    ...
    if (iterFind == existingEntries.end()) {
      if (type == AssetType::Normal) {
        ...
        nlohmann::json newEntry = {{"Path", relativePath.toString().c_str()},
                                   {"UUID", uuid.c_str()}};
...
void BuiltinResourcesHelper::updateManifest(
    const Path &folder, const nlohmann::json &entries,
  ...
  for (auto &entry : entries) {
    ...
    if (isIcon) {
      ...
      if (type == AssetType::Normal) {
        texUUIDs[0] = entry.value("UUID48", "");
        texUUIDs[1] = entry.value("UUID32", "");
        texUUIDs[2] = entry.value("UUID16", "");
      } else if (type == AssetType::Sprite) {
        texUUIDs[0] = entry.value("TextureUUID48", "");
        texUUIDs[1] = entry.value("TextureUUID32", "");
        texUUIDs[2] = entry.value("TextureUUID16", "");
      }
      ...
      if (type == AssetType::Sprite) {
        ...
        spriteUUIDs[0] = entry.value("SpriteUUID48", "");
        spriteUUIDs[1] = entry.value("SpriteUUID32", "");
        spriteUUIDs[2] = entry.value("SpriteUUID16", "");
...
UINT32 BuiltinResourcesHelper::checkForModifications(const Path &folder,
...
bool BuiltinResourcesHelper::verifyAndReportShader(const HShader &shader) {
  if (!shader.isLoaded(false) || shader->getNumTechniques() == 0) {
    ...
    BS_EXCEPT(InvalidStateException,
              "Error occured while compiling a shader. Check earlier log "
              "messages for exact error.");
#else
    BS_LOG(Error, Importer,
           "Error occured while compiling a shader. Check earlier log messages "
           "for exact error.");
#endif
    return false;
  }
  ...
  for (auto &technique : techniques) {
    ...
    for (UINT32 i = 0; i < numPasses; i++) {
      ...
      const SPtr<GraphicsPipelineState> &graphicsPipeline =
          pass->getGraphicsPipelineState();
      if (graphicsPipeline) {
      ...
      const SPtr<ComputePipelineState> &computePipeline =
          pass->getComputePipelineState();
      if (computePipeline)
      ...
      for (auto &program : gpuPrograms) {
        ...
        if (!program->isCompiled()) {
          String errMsg =
              "Error occured while compiling a shader \"" + shader->getName() +
...
void BuiltinResourcesHelper::updateShaderBytecode(const Path &path) {
  HShader shader =
      gResources().load<Shader>(path, ResourceLoadFlag::KeepSourceData);
  if (!shader)
    return;
...
GUIElementStyle BuiltinResourcesHelper::loadGUIStyleFromJSON(
    const nlohmann::json &entry, const GUIElementStyleLoader &loader) {
  ...
  const auto loadState = [&loader, &entry](const char *name,
                                           GUIElementStateStyle &state) {
...
BuiltinResourceGUIElementStyleLoader::BuiltinResourceGUIElementStyleLoader(
    const Path &fontPath, const Path &texturePath)
    : mFontPath(fontPath), mTexturePath(texturePath) {}

HSpriteTexture
BuiltinResourceGUIElementStyleLoader::loadTexture(const String &name) const {
